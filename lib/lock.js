...
// compatible, but we lose precision
var LOCK_FLAG_VALUE_IN = '3304740253564472344';
var LOCK_FLAG_VALUE_OUT = parseInt(LOCK_FLAG_VALUE_IN, 10);
...
Lock.prototype.acquire = function() {
  ...
  if (self._ctx) {
    return self._err(errors.Validation('lock in use'));
  }
...
Lock.prototype.release = function() {
  ...
  if (!ctx) {
    return self._err(errors.Validation('no lock in use'));
  }
...
Lock.prototype._wait = function(ctx) {
  ...
  self.consul.kv.get(opts, function(err, data, res) {
    ...
    if (data) {
      if (data.Flags !== LOCK_FLAG_VALUE_OUT) {
      ...
      if (newIndex > ctx.index) ctx.index = newIndex;
