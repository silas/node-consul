...
function request(options, callback) {
  ...
  options.emit('debug', 'request', opts);

  var req = transport.request(opts);

  req.on('error', function(err) {
    options.emit('debug', 'request.error', err);
  ...
  req.on('response', function(res) {
    ...
    res.on('end', function() {
      ...
      options.emit('debug', 'response.statusCode', res.statusCode);
      options.emit('debug', 'response.headers', res.headers);

      if (chunks.length) {
        res.body = Buffer.concat(chunks, bodyLength);

        options.emit('debug', 'response.body', res.body);
